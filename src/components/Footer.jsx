      ...
      <div className="container">
        <div className="footer-top">
          <div className="footer-brand">
            <h3>{t('footer.brand')} <span className="highlight">{t('footer.brand_highlight')}</span></h3>
            <p>{t('footer.tagline')}</p>
            <div className="footer-social-links">
              <a href="https://www.facebook.com/BahirDarAC" target="_blank" rel="noopener noreferrer" className="social-icon" aria-label="Facebook">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
              </a>
              <a href="https://t.me/BahirDarAC" target="_blank" rel="noopener noreferrer" className="social-icon" aria-label="Telegram">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1 .22-1.59.15-.15 2.71-2.48 2.76-2.69.01-.03.01-.14-.07-.2-.08-.06-.19-.04-.27-.02-.11.02-1.93 1.23-5.46 3.62-.51.35-.98.53-1.39.51-.46-.01-1.33-.26-1.98-.48-.8-.27-1.43-.42-1.37-.89.03-.25.38-.51 1.03-.78 4.04-1.75 6.73-2.91 8.07-3.47 3.84-1.6 4.63-1.88 5.15-1.89.11 0 .37.03.54.17.14.12.18.28.2.45-.02.07-.02.13-.02.26z"/>
                </svg>
              </a>
          ...
          <div className="footer-social">
            ...
            <div style={{ color: '#94a3b8', fontSize: '0.95rem' }}>
              <p style={{ marginBottom: '10px' }}>{t('footer.mon_fri')}</p>
              <p style={{ marginBottom: '10px' }}>{t('footer.sat')}</p>
              <p style={{ fontStyle: 'italic', opacity: 0.8 }}>{t('footer.lunch_break')}</p>
            </div>
          </div>
        </div>

        <div className="footer-bottom">
          <p>© {new Date().getFullYear()} <strong>{t('footer.rights')}</strong></p>
        </div>
      </div>
    </footer>
  );
